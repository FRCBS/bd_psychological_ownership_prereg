...
data.fit <- data %>%
  ...
  mutate(
    ...
    ss_intention = 
      ...
      rowMeans(na.rm = TRUE) %>% 
      scale() %>% as.numeric(),
    ss_control = across(starts_with("ac"),~as.numeric(.)) %>%  
      rowMeans(na.rm = TRUE) %>% 
      scale() %>% as.numeric(),
    ss_knowledge = across(starts_with("ai"),~as.numeric(.)) %>%  
      rowMeans(na.rm = TRUE) %>% 
      scale() %>% as.numeric(),
    ss_selfinv = across(starts_with("iv"),~as.numeric(.)) %>%  
      rowMeans(na.rm = TRUE) %>% 
      scale() %>% as.numeric(),
...
data.fit2 <- data.fit %>% 
  #standarsing behaviour variables
  mutate(across(c(bd02, bd03, bd04), ~scale(.) %>% as.numeric())) %>% 
  #remove these as they are not used in modelling
  select(
    -ss_control,
    -ss_knowledge,
    ss_selfinv
...
tmp <- data.fit %>% 
  ...
  rename(
        "Gender"=de01,
         "Age (years)" = de02,
         "Control" = ss_control,
         "Knowledge" =  ss_knowledge,
         "Self-investment" = ss_selfinv,
         "Past donations in 2 years" = bd03, 
         "Psychological ownership" = ss_psycown,
         "Self-identity" = ss_selfid,
         "Intention" = ss_intention 
           )

pt2 <- ggcorrplot(
  #compute correlatins from normalised values
  cor(tmp,use="pairwise.complete.obs"),
  show.diag = FALSE,
  outline.color = "white",
  p.mat = cor_pmat(tmp,use="pairwise.complete.obs"),
  sig.level=0.01,
  #insig = "",
  lab=TRUE,
  lab_size = 2,
...
#compute mean and sd on original scale
tmp <- data %>%
  #lavaan cannot deal with unordered factor with more then 2 levels
  filter(de01 == 'man' | de01 == "woman") %>% # EXCLUSION
  filter(bd01 != 'no') %>% # EXCLUSION
  mutate(
    across(isfactor, ~ fct_drop(.x)
      ...
      mutate(
        de01 = as.numeric(de01), # for counting correlation
       )
  ) %>% 
  #Original paper used sum scores instead of FA. 
  mutate(
    ...
    de01,de02,ss_control,ss_knowledge,ss_selfinv,bd03, ss_psycown,ss_selfid,ss_intention 
  ) %>% pivot_longer(cols = everything(), names_to = "Variable", values_to = "val") %>%
  group_by(Variable) %>% 
  summarize(mean = mean(val,na.rm=TRUE), sd = sd(val,na.rm=TRUE)
  ) %>% mutate(
    Mean = as.character(round(mean,2)),
    SD = as.character(round(sd,2))
  ) %>% select(-mean,-sd) %>%  
  mutate(
    Variable = factor(
      Variable,
...
tmp[tmp$Variable == 'de01','Mean'] <- ' ' 
tmp[tmp$Variable == 'de01','SD'] <- ' '

#Plot tehm
tab1 <-  gt(tmp %>% select(-x)) %>% tab_options(table.font.size = 12,data_row.padding = px(6))
pt2 + wrap_table(tab1,space="free_y")
```

```{r}
ggsave(filename = here("figures/table1asFig.pdf"),units = "cm",width = 18,height = 10)
ggsave(filename = here("figures/table1asFig.png"),units = "cm",width = 18,height = 10)
```
...
#regression
ss_intention ~  de01 + de02 + cp * ss_donation + b1 * ss_psycown + b2 * ss_selfid
...
labelsforplottingnull <- c(
  ...
  "ss_donation" = "Past donation behaviour"
)
...
labelsforplotting <- c(
  ...
  "intention" = "Intention",
  "donation" = "Past donation behaviour"
)
...
#Previously Reported Effects (Sum score, bootstrapped, listwise)
...
manual_data1 <- tibble(
  ...
  label = c("manual1a1", "manual1a2", "manual1b1", "manual1b2", "manual1d21", "manual1cp", "manual1eff", "manual1eff1", "manual1eff2"),
...
#best practice model (latent, robust, fiml)
manual_data2 <- tibble(
  est = c(0.369, 0.373, -0.19, 0.825, 0.341, -0.089, 0.104, -0.07, 0.308),
  ci.lower = c(0.25, 0.279, -0.298, 0.696, 0.22, -0.183, 0.048, -0.117, 0.23),
  ci.upper = c(0.487, 0.467, -0.081, 0.954, 0.461, 0.004, 0.159, -0.023, 0.386),
  label = c("manual2a1", "manual2a2", "manual2b1", "manual2b2", "manual2d21", "manual2cp", "manual2eff", "manual2eff1", "manual2eff2"),
  lhs = c("psycown", "selfid", "intention", "intention", "selfid", "intention", "ind_eff", "ind_eff1", "ind_eff2"),
  rhs = c("donation", "donation", "psycown", "selfid", "psycown", "donation", "a1*d21*b2", "a1*b1", "a2*b2"),
  op = c("~", "~", "~", "~", "~", "~", ":=", ":=", ":="),
  term = c("a1:psycown~donation", "a2:selfid~donation", "b1:intention~psycown", "b2:intention~selfid", "d21:selfid~psycown", "cp:intention~donation", "ind_eff:=a1*d21*b2", "ind_eff1:ind_eff1:=a1*b1", "ind_eff2:ind_eff2:=a2*b2"),
...
data.forest <- bind_rows(data.forest, manual_data1, manual_data2)
...
pf <- data.forest %>% 
  ...
  geom_linerange(aes(size = sample), position=pos) +
  scale_size_manual(values = c("Australia" = 1.2, "Finland" = 0.8)) +
  geom_point(aes(shape = sample),size=3,position=pos,alpha=0.7)+
  scale_fill_discrete(na.value=NA, guide ="none") + 
  scale_y_discrete(
    name = "Relationship", 
    labels = c(
      "ind_eff2:ind_eff2:=a2*b2" = "Indirect Effect: a2*b2", 
      "ind_eff1:ind_eff1:=a1*b1" = "Indirect Effect: a1*b1",
      "ind_eff:=a1*d21*b2" = "Indirect Effect: a1*d21*b1",
      "d21:selfid~psycown" = "Identity*Ownership (d21)",
      "cp:intention~donation" = "Intention*Donation (cp)",
      "b2:intention~selfid" = "Intention*Identity (b2)",
      "b1:intention~psycown" = "Intention*Owenrship (b1)",
      "a2:selfid~donation" = "Identity*Donation (a2)",
      "a1:psycown~donation" = "Ownership*Donation(a1)"
    )
  ) +
  labs(x="Standardised Effect Size",y="Relationship") +
  labs(color="Model type", linetype="Variable type", shape="Sample") + 
  guides(size = "none") +
