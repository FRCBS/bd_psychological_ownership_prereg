...
#additional packages added (plotting, cronbach's alpha)
library(naniar)
library(psych)
library(gridExtra)
...
```
##unstandarsied means for reporting

```{r}

data2 <- data %>%
mutate(across(c(bd02, bd03, bd04), as.numeric))
...
data.fit.stan <- data2 %>%
  ...
  mutate(
    ...
    ss_intention = 
      ...
      rowMeans(na.rm = TRUE),
    ss_control = 
      across(starts_with("ac"),~as.numeric(.)) %>%  
      rowMeans(na.rm = TRUE),
    ss_knowledge = 
      across(starts_with("ai"),~as.numeric(.)) %>%  
      rowMeans(na.rm = TRUE),
    ss_investment = 
      across(starts_with("iv"),~as.numeric(.)) %>%  
      rowMeans(na.rm = TRUE)
  )
...
data.fit.stan %>%
  summarise(across(
    c(de02, ss_control, ss_knowledge, ss_investment, bd03,
      ss_psycown, ss_selfid, ss_intention),
...
## Exploratory analysis: missingness on ei01 variable 

```{r}
#, labels = c('miss', 'not'), ordered = TRUE

data.fit.miss <- data.fit2 %>%
  mutate(
    ss_control = 
      across(starts_with("ac"),~as.numeric(.)) %>%  
      rowMeans(na.rm = TRUE) %>% 
...
data.fit.miss$ie01bin <- ifelse(is.na(data.fit2$ie01), 1, 2)
data.fit.miss$ie01bin <- factor(data.fit.miss$ie01bin, levels = c(1, 2), labels = c("miss", "not"), ordered = TRUE)
summary(data.fit.miss)


data.fit.miss <- data.fit.miss %>% 
  mutate(
    ie01bin = as.ordered(ie01bin)
  ) %>% # and neither any factors at all
mutate(
  across(everything(),as.numeric)
)  # but this also good for plotting correlations
...
corrplot(cor(data.fit.miss,use="pairwise.complete.obs"), method ='number')

cor_test <- corr.test(data.fit.miss, use = "pairwise", method = "pearson")

# Combine correlation and p-values into APA-style strings
apa_matrix <- matrix("", nrow=ncol(data.fit.miss), ncol=ncol(data.fit.miss))
rownames(apa_matrix) <- colnames(data.fit.miss)
colnames(apa_matrix) <- colnames(data.fit.miss)

for (i in 1:ncol(data.fit.miss)) {
  for (j in 1:ncol(data.fit.miss)) {
    r_val <- round(cor_test$r[i, j], 2)
    p_val <- cor_test$p[i, j]
    
    # Format p-value with APA rules
    p_str <- ifelse(p_val < .001, "< .001", paste0("= ", round(p_val, 3)))
    
    # Combine r and p
    apa_matrix[i, j] <- paste0(r_val, ", _p_ ", p_str)
  }
}
...
scalepo <- data.fit2 %>% select(starts_with("po"))
scaleid <- data.fit2 %>% select(starts_with("si"))
scaleint <- data.fit2 %>% select(starts_with("it"))
scaleco <- data.fit2 %>% select(starts_with("ac"))
scalekn <- data.fit2 %>% select(starts_with("ai"))
scaleiv <- data.fit2 %>% select(starts_with("iv"))
...
## Factor Scores: a1*b1 comparisos
...
factor_scores <- lavPredict(fit.po04, type = "lv")
head(factor_scores)
...
factor1 <- ggplot(df_scores, aes(x = psycown, y = blooddonor)) +
  ...
  labs(title = "Factor scores: psycown ~ blooddonor",
       x = "Ownership",
       y = "Donation history",
       color = "Age",
       linetype = "Gender") +
...
factor2 <- ggplot(df_scores, aes(x = selfid, y = blooddonor)) +
  geom_point(aes(color = de02), alpha = 0.7, size = 2.5) +
  geom_smooth(aes(linetype = de01), colour = "black", se = FALSE) +
  labs(title = "Factor scores: selfid ~ blooddonor",
       x = "Identity",
       y = "Donation history",
       color = "Age",
       linetype = "Gender") +
...
factor3 <- ggplot(df_scores, aes(x = psycown, y = selfid)) +
  geom_point(aes(color = de02), alpha = 0.7, size = 2.5) +
  geom_smooth(aes(linetype = de01), colour = "black", se = FALSE) +
  labs(title = "Factor scores: selfid ~ psycown",
       x = "Ownership",
       y = "Identity",
       color = "Age",
       linetype = "Gender") +
...
factor4 <- ggplot(df_scores, aes(x = intention, y = blooddonor,
  ...
  geom_point(aes(color = de02), alpha = 0.7, size = 2.5) +
  geom_smooth(aes(linetype = de01), colour = "black", se = FALSE) +
  labs(title = "Factor scores: intention ~ blooddonor",
       x = "Intention",
       y = "Donation History",
       color = "Age",
       linetype = "Gender") +
...
factor5 <- ggplot(df_scores, aes(x = psycown, y = intention,
  ...
  geom_point(aes(color = de02), alpha = 0.7, size = 2.5) +
  geom_smooth(aes(linetype = de01), colour = "black", se = FALSE) +
  labs(title = "Factor Scores: psycown ~ intention",
       x = "Ownership",
       y = "Intention",
       color = "Age",
       linetype = "Gender") +
...
#intention ~ selfid
factor6 <- ggplot(df_scores, aes(x = selfid, y = intention,
                      color = as.factor(de01))) +
  geom_point(aes(color = de02), alpha = 0.7, size = 2.5) +
  geom_smooth(aes(linetype = de01), colour = "black", se = FALSE) +
  labs(title = "Factor Scores: selfid ~ intention",
       x = "Identity",
       y = "Intention",
       color = "Age",
       linetype = "Gender") +
...
# Multigroup analysis to explore impact of gender
...
intention ~ c(cpg1, cpg2) * blooddonor + c(b1g1, b1g2) * psycown + c(b2g1, b2g2) * selfid
...
psycown ~ a1 * blooddonor
selfid ~ a2 * blooddonor + d21 * psycown
intention ~ cp * blooddonor + b1 * psycown + b2 * selfid
...
lavTestLRT(ea2.free.fit, ea2.fixed.fit)
