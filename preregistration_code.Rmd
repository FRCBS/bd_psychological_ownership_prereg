...
data.forest  <- fit.po03.best %>% 
  ...
  filter(op == "~" | op == ":=") %>% 
  bind_rows(
    fit.po02.est %>% mutate(estmet = "Robust") %>% 
      rename(est=est.std) %>% 
      filter(op == "~"| op == ":="),
    fit.po04.est %>% mutate(estmet = "Imputed") %>% 
      rename(est=est.std) %>% 
      filter(op == "~"| op == ":=") 
  ) %>% 
  mutate(
...
data.forest  <- fit.po03ss.best %>% 
  ...
  filter(op == "~"| op == ":=") %>% 
  bind_rows(
    fit.po02ss.est %>% mutate(estmet = "Robust") %>% 
      rename(est=est.std) %>% 
      filter(op == "~"| op == ":=") ,
    fit.po04ss.est %>% mutate(estmet = "Imputed") %>% 
      rename(est=est.std) %>% 
      filter(op == "~"| op == ":=")
  ...
  mutate(
    mtype = str_c(estmet,"-",linetype)
  )
  



pos <- position_nudge(y=as.numeric(as.factor(data.forest$mtype))/7 -0.5) 

pf <- data.forest %>% 
  ggplot(aes(est,xmin=ci.lower,xmax=ci.upper,y=term,fill=hollow_group,color=estmet,linetype = linetype))+
